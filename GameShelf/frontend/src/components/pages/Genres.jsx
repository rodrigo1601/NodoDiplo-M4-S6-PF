...
const Genres = () => {
    useDocumentTitle("Géneros | GameShelf");
    ...
    return (
        <div className={`min-h-screen font-sans transition-colors ${isDark ? "bg-[#0f1117] text-white" : "bg-gray-50 text-gray-900"}`}>
            <div className={`border-b px-6 py-6 ${isDark ? "border-white/4" : "border-gray-200"}`}>
                ...
                <h1 className="text-2xl font-extrabold tracking-tight">Géneros</h1>
                <p className={`text-sm mt-0.5 ${isDark ? "text-gray-500" : "text-gray-400"}`}>
                    Encontrá tu próximo juego según el tipo de experiencia que buscás.
                </p>
            </div>
                        ...
                        <>
                            <div className="flex items-center mb-4">
                                <span className={`text-xs ${isDark ? "text-gray-600" : "text-gray-400"}`}>
                                    <span className="text-violet-500 font-semibold">{genres.length}</span> géneros
                                </span>
                            </div>
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                                {genres.map(genre => (
                                    <CardItem key={genre._id} item={genre} type="genres" />
                                ))}
                            </div>
                        </>
                    )}
